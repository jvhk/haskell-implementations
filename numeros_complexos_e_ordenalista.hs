--Soma
soma :: (Double,Double) -> (Double,Double) -> (Double,Double)
soma (x,y) (a,b) = (x+a,y+b)


--Subtracao
subtracao :: (Double,Double) -> (Double,Double) -> (Double,Double)
soma (x,y) (a,b) = (x-a,y-b)


--Multiplicacao
multiplicacao :: (Double,Double) -> (Double,Double) -> (Double,Double)
multiplicacao (x,y) (a,b) = (x*a-y*b,x*b+y*a)

--Divisao
divisao :: (Double,Double) -> (Double,Double) -> (Double,Double) 
divisao (a,b) (c,d) =(((a*c+b*d)/(c*c+d*d)),((c*b-a*d)/(c*c+d*d)))





--Ordena lista
maior :: [Int] -> Int
maior [x] = x
maior (cabeca:calda) | (cabeca > maior calda ) = cabeca
                     | otherwise = maior calda

remove_maior :: [Int] -> [Int]
remove_maior [] = []
remove_maior (x:xs) | (x == maior(x:xs)) = xs
                    | otherwise = x:remove_maior xs

ordena :: [Int] -> [Int]
ordena [] = []
ordena [x] = [x]
ordena (x:xs) = ordena (remove_maior (x:xs)) ++ [maior (x:xs)]




